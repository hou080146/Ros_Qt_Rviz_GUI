...
//    读取topic的设置
    ...
    odom_topic= topic_setting.value("topic_odom","raw_odom").toString();//key:topic_odom  defaultValue:raw_odom
...
//发布楼层话题
void QNode::set_floor(int8_t floor)
{
    std_msgs::Int8 msg;
    msg.data = floor;
    floor_pub.publish(msg);
    ros::spinOnce();
}
...
//发布机器人速度控制
 ...
 //订阅图片话题，并在label上显示  void MainWindow::initVideos()中调用
